...
require_once $incPath."/facebook_php_sdk/facebook.php";
...
{
	htmlHeader(" - Post to Social Networking");
	// Display admin options if we are one
	$adminOptions = getAdminOptions();
	// Get form
	$form = htmlOutput("../tmpl/man/tinyurl.txt",array(),array(),true);
	// Display
	htmlOutput("../tmpl/man/main.txt",array("title","form","admin"),array("Post to Social Networking",$form,$adminOptions));
	htmlFooter();
}
